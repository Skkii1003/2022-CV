...
nx = 9
ny = 6
file_paths = glob.glob("./Camera_Calibration/camera_cal/calibration*.jpg")
...
if __name__ == "__main__":
    ...
    # 测试车道线提取
    img = cv2.imread('sample/test/test1.jpg')
    result = pipeline(img)
    plot_contrast_image(img, result, converted_img_gray=True)

    # 测试透视变换
    img = cv2.imread('sample/test/straight_lines2.jpg')
...
# 视频处理
clip1 = VideoFileClip("sample/video/6.mp4")
white_clip = clip1.fl_image(process_image)
# white_clip.ipython_display()
white_clip.write_videofile("output/video/6_out.mp4", audio=False)